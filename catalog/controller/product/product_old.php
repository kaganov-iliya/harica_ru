...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if ($product_info) {
			...
			$data['yt_video'] = html_entity_decode($product_info['yt_video'], ENT_QUOTES, 'UTF-8');
			...
			$data['entry_yt_video'] = $this->language->get('entry_yt_video');
				...
				foreach ($option['product_option_value'] as $option_value) {
					if (!$option_value['subtract'] || ($option_value['quantity'] > 0)) {
						...
						$product_option_value_data[] = array(
							...
							'thumb'                   => $this->model_tool_image->resize($option_value['image'], 495, 495),
							'price'                   => $price,
...
### download file to product
			...
			foreach ($product_downloads as $product_download) {
				...
				$download = $this->model_catalog_product->getDownload($product_download['download_id']);
					...
					if (file_exists(DIR_DOWNLOAD . $download['filename'])) {
						...
						$data['downloads'][] = array(
							...
							'href'       => $this->url->link('product/product/download', 'download_id=' . $product_download['download_id'], 'SSL')
						);
					}				
			}
###
	...
	public function review($incoming = array()) {
		$this->load->language('product/product');
		...
		$direct = false;
		if (isset($incoming['product_id'])) {
			$product_id = (int)$incoming['product_id'];
			$direct = true;
	...
	public function write() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			if ((utf8_strlen($this->request->post['name']) < 10) || (utf8_strlen($this->request->post['name']) > 50)) {
				$json['error'] = $this->language->get('error_name');
			}

			if ((utf8_strlen($this->request->post['text']) < 25) || (utf8_strlen($this->request->post['text']) > 5000)) {
