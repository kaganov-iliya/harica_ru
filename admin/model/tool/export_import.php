...
class ModelToolExportImport extends Model {
	...
	protected function storeCategoryIntoDatabase( &$category, &$languages, $exist_meta_title, &$layout_ids, &$available_store_ids, &$url_alias_ids ) {
		...
		/* ocdesign.ru */
		$custom_titles = $category['custom_titles'];
		$seo_texts = $category['seo_texts'];
		/* ocdesign.ru */
		...
		foreach ($languages as $language) {
			...
			/* ocdesign.ru */
			$custom_title = isset($custom_titles[$language_code]) ? $this->db->escape($custom_titles[$language_code]) : '';
			$seo_text = isset($seo_texts[$language_code]) ? $this->db->escape($seo_texts[$language_code]) : '';
			
			if ($exist_meta_title) {
				$sql  = "INSERT INTO `".DB_PREFIX."category_description` (`category_id`, `language_id`, `name`, `description`, `meta_title`, `meta_description`, `meta_keyword`, `custom_title`, `seo_text`) VALUES ";
				$sql .= "( $category_id, $language_id, '$name', '$description', '$meta_title', '$meta_description', '$meta_keyword', '$custom_title', '$seo_text' );";
			} else {
				$sql  = "INSERT INTO `".DB_PREFIX."category_description` (`category_id`, `language_id`, `name`, `description`, `meta_description`, `meta_keyword`, `custom_title`, `seo_text`) VALUES ";
				$sql .= "( $category_id, $language_id, '$name', '$description', '$meta_description', '$meta_keyword' , '$custom_title' , '$seo_text' );";
			}
			/* ocdesign.ru */
	...
	protected function uploadCategories( &$reader, $incremental, &$available_category_ids=array() ) {
		...
		for ($i=0; $i<$k; $i+=1) {
			...
			/* ocdesign.ru */
			$custom_titles = array();
			while ($this->startsWith($first_row[$j-1],"custom_titles(")) {
				$language_code = substr($first_row[$j-1],strlen("custom_titles("),strlen($first_row[$j-1])-strlen("custom_titles(")-1);
				$custom_title = $this->getCell($data,$i,$j++);
				$custom_title = htmlspecialchars( $custom_title );
				$custom_titles[$language_code] = $custom_title;
			}			
			$seo_texts = array();
			while ($this->startsWith($first_row[$j-1],"seo_texts(")) {
				$language_code = substr($first_row[$j-1],strlen("seo_texts("),strlen($first_row[$j-1])-strlen("seo_texts(")-1);
				$seo_text = $this->getCell($data,$i,$j++);
				$seo_text = htmlspecialchars( $seo_text );
				$seo_texts[$language_code] = $seo_text;
			}
			/* ocdesign.ru */
			...
			/* ocdesign.ru */
			$category['custom_titles'] = $custom_titles;
			$category['seo_texts'] = $seo_texts;
			/* ocdesign.ru */
	...
	protected function storeProductIntoDatabase( &$product, &$languages, &$product_fields, $exist_table_product_tag, $exist_meta_title, &$layout_ids, &$available_store_ids, &$manufacturers, &$weight_class_ids, &$length_class_ids, &$url_alias_ids ) {
		...
		foreach ($languages as $language) {
			...
			$feature = isset($features[$language_code]) ? $this->db->escape($features[$language_code]) : '';
			$yt_video = isset($yt_videos[$language_code]) ? $this->db->escape($yt_videos[$language_code]) : '';
			if ($exist_table_product_tag) {
				if ($exist_meta_title) {
					$sql  = "INSERT INTO `".DB_PREFIX."product_description` (`product_id`, `language_id`, `name`, `description`, `meta_title`, `meta_description`, `meta_keyword`, `features`, `yt_video`) VALUES ";
					$sql .= "( $product_id, $language_id, '$name', '$description', '$meta_title', '$meta_description', '$meta_keyword', '$feature', '$yt_video' );";
				} else {
					$sql  = "INSERT INTO `".DB_PREFIX."product_description` (`product_id`, `language_id`, `name`, `description`, `meta_description`, `meta_keyword`, `features`, `yt_video`) VALUES ";
					$sql .= "( $product_id, $language_id, '$name', '$description', '$meta_description', '$meta_keyword', '$feature', '$yt_video' );";
				}
				$this->db->query( $sql );
				...
				if ($exist_meta_title) {
					$sql  = "INSERT INTO `".DB_PREFIX."product_description` (`product_id`, `language_id`, `name`, `description`, `meta_title`, `meta_description`, `meta_keyword`, `tag`, `features`, `yt_video`) VALUES ";
					$sql .= "( $product_id, $language_id, '$name', '$description', '$meta_title', '$meta_description', '$meta_keyword', '$tag', '$feature', '$yt_video' );";
				} else {
					$sql  = "INSERT INTO `".DB_PREFIX."product_description` (`product_id`, `language_id`, `name`, `description`, `meta_description`, `meta_keyword`, `tag`, `features`, `yt_video`) VALUES ";
					$sql .= "( $product_id, $language_id, '$name', '$description', '$meta_description', '$meta_keyword', '$tag', '$feature', '$yt_video' );";
				}
				$this->db->query( $sql );
			}
		}
		if (count($categories) > 0) {
			$sql = "INSERT INTO `".DB_PREFIX."product_to_category` (`product_id`,`category_id`, `main_category`) VALUES ";
			$first = true;
			foreach ($categories as $category_id) {
                $m_cat = (int)$main_category == (int)$category_id ? 1:0;
				$sql .= ($first) ? "\n" : ",\n";
				$first = false;
				$sql .= "($product_id,$category_id,$m_cat)";
			}
			$sql .= ";";
			$this->db->query($sql);
		}
	...
	protected function uploadProducts( &$reader, $incremental, &$available_product_ids=array() ) {
		...
		for ($i=0; $i<$k; $i+=1) {
			...
			$main_category = $this->getCell($data,$i,$j++);
			...
			$features = array();
			while ($this->startsWith($first_row[$j-1],"features(")) {
				$language_code = substr($first_row[$j-1],strlen("features("),strlen($first_row[$j-1])-strlen("features(")-1);
				$feature = $this->getCell($data,$i,$j++);
				$feature = htmlspecialchars( $feature );
				$features[$language_code] = $feature;
			}

			$yt_videos = array();
			while ($this->startsWith($first_row[$j-1],"yt_video(")) {
				$language_code = substr($first_row[$j-1],strlen("yt_video("),strlen($first_row[$j-1])-strlen("yt_video(")-1);
				$yt_video = $this->getCell($data,$i,$j++);
				$yt_video = htmlspecialchars( $yt_video );
				$yt_videos[$language_code] = $yt_video;
			}
	...
	protected function validateCategories( &$reader ) {
		...
		if ($exist_meta_title) {
			...
			( "category_id", "parent_id", "name", "top", "columns", "sort_order", "image_name", "date_added", "date_modified", "seo_keyword", "description", "meta_title", "meta_description", "meta_keywords", "custom_titles", "seo_texts", "store_ids", "layout", "status" );
			$expected_multilingual = array( "name", "description", "meta_title", "meta_description", "meta_keywords", "custom_titles", "seo_texts" );
		} else {
			$expected_heading = array
			( "category_id", "parent_id", "name", "top", "columns", "sort_order", "image_name", "date_added", "date_modified", "seo_keyword", "description", "meta_description", "meta_keywords", "custom_titles", "seo_texts", "store_ids", "layout", "status" );
			$expected_multilingual = array( "name", "description", "meta_description", "meta_keywords" , "custom_titles" , "seo_texts" );
		}
		/* ocdesign.ru */
	...
	protected function validateProducts( &$reader ) {
		...
		$expected_heading = array
		( "product_id", "name", "categories", "main_category", "sku", "upc" );
		if (in_array("ean",$product_fields)) {
		...
		$expected_heading = array_merge( $expected_heading, array( "meta_description", "meta_keywords",  "features", "yt_video", "stock_status_id", "store_ids", "layout", "related_ids", "tags", "sort_order", "subtract", "minimum" ) );
		if ($exist_meta_title) {
			$expected_multilingual = array( "name", "description", "meta_title", "meta_description", "meta_keywords", "features", "yt_video", "tags" );
		} else {
			$expected_multilingual = array( "name", "description", "meta_description", "meta_keywords",  "features", "yt_video", "tags" );
		}
		return $this->validateHeading( $data, $expected_heading, $expected_multilingual );
	}
...
//		}
		...
		foreach ($languages as $language) {
			...
			foreach ($results->rows as $key=>$row) {
				if (isset($category_descriptions[$language_code][$key])) {
					...
					/* ocdesign.ru */
					$results->rows[$key]['custom_title'][$language_code] = $category_descriptions[$language_code][$key]['custom_title'];
					$results->rows[$key]['seo_text'][$language_code] = $category_descriptions[$language_code][$key]['seo_text'];
					/* ocdesign.ru */
					...
					/* ocdesign.ru */
					$results->rows[$key]['custom_title'][$language_code] = '';
					$results->rows[$key]['seo_text'][$language_code] = '';
					/* ocdesign.ru */
				}
			}
		}
	...
	protected function populateCategoriesWorksheet( &$worksheet, &$languages, &$box_format, &$text_format, $offset=null, $rows=null, &$min_id=null, &$max_id=null ) {
		...
		/* ocdesign.ru */
		foreach ($languages as $language) {
			$worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('custom_titles'),32)+1);
		}		
		foreach ($languages as $language) {
			$worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('seo_texts'),32)+1);
		}
		/* ocdesign.ru */
		...
		foreach ($languages as $language) {
			$styles[$j] = &$text_format;
			$data[$j++] = 'custom_titles('.$language['code'].')';
		}		
		foreach ($languages as $language) {
			$styles[$j] = &$text_format;
			$data[$j++] = 'seo_texts('.$language['code'].')';
		}
		/* ocdesign.ru */
		...
		foreach ($categories as $row) {
			...
			/* ocdesign.ru */
			foreach ($languages as $language) {
				$data[$j++] = html_entity_decode($row['custom_title'][$language['code']],ENT_QUOTES,'UTF-8');
			}			
			foreach ($languages as $language) {
				$data[$j++] = html_entity_decode($row['seo_text'][$language['code']],ENT_QUOTES,'UTF-8');
			}
			/* ocdesign.ru */
	...
	protected function getProducts( &$languages, $default_language_id, $product_fields, $exist_meta_title, $offset=null, $rows=null, $min_id=null, $max_id=null ) {
		...
		$sql .= " (SELECT category_id  FROM `".DB_PREFIX."product_to_category` WHERE main_category = 1 AND product_id = p.product_id ) as main_category, ";
		$sql .= "  p.sku,";
		$sql .= "  p.upc,";
		if (in_array( 'ean', $product_fields )) {
			$sql .= "  p.ean,";
		...
		foreach ($languages as $language) {
			...
			foreach ($results->rows as $key=>$row) {
				if (isset($product_descriptions[$language_code][$key])) {
					...
					$results->rows[$key]['features'][$language_code] = $product_descriptions[$language_code][$key]['features'];
					$results->rows[$key]['yt_video'][$language_code] = $product_descriptions[$language_code][$key]['yt_video'];
				} else {
					$results->rows[$key]['name'][$language_code] = '';
					$results->rows[$key]['description'][$language_code] = '';
					if ($exist_meta_title) {
					...
					$results->rows[$key]['features'][$language_code] = '';
					$results->rows[$key]['yt_video'][$language_code] = '';
				}
			}
		}
		return $results->rows;
	...
	function populateProductsWorksheet( &$worksheet, &$languages, $default_language_id, &$price_format, &$box_format, &$weight_format, &$text_format, $offset=null, $rows=null, &$min_id=null, &$max_id=null) {
		...
		$worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('main_category'),12)+1);
		...
		foreach ($languages as $language) {
			$worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('features')+4,32)+1);
		}
		foreach ($languages as $language) {
			$worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('yt_video')+4,32)+1);
		}
		...
		foreach ($languages as $language) {
			$styles[$j] = &$text_format;
			$data[$j++] = 'features('.$language['code'].')';
		}
		foreach ($languages as $language) {
			$styles[$j] = &$text_format;
			$data[$j++] = 'yt_video('.$language['code'].')';
		}
		...
		foreach ($products as $row) {
			...
			foreach ($languages as $language) {
				$data[$j++] = html_entity_decode($row['features'][$language['code']],ENT_QUOTES,'UTF-8');
			}
			foreach ($languages as $language) {
				$data[$j++] = html_entity_decode($row['yt_video'][$language['code']],ENT_QUOTES,'UTF-8');
			}
	...
	public function getNotifications() {
		...
		$result = $this->curl_get_contents("http://www.mhccorp.com/index.php?route=information/message&type=tool_export_import_2_30&language_code=$language_code");
